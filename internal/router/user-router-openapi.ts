...
import { UserSchema, CreateUserSchema, UpdateUserSchema, ErrorSchema } from '../schemas/user-schema.js'
...
export function createUserRouter(userUseCase: UserUseCase) {
  ...
  const getAllUsersRoute = createRoute({
    ...
    tags: ['Users'],
    summary: 'すべてのユーザーを取得',
    description: 'すべてのユーザー情報を取得します。削除済みユーザーを含めることも可能です。',
    request: {
      query: z.object({
        includeDeleted: z.string().optional().openapi({ example: 'false', description: '削除済みユーザーを含める場合はtrue' })
      })
    },
    responses: {
      200: {
  ...
  const getUserByIdRoute = createRoute({
    ...
    request: {
      params: z.object({
        id: z.string().openapi({ example: '123e4567-e89b-12d3-a456-426614174000', description: 'ユーザーID' })
      })
    },
    responses: {
      200: {
  ...
  const getUserByMailRoute = createRoute({
    ...
    request: {
      params: z.object({
        mail: z.string().openapi({ example: 'yamada@example.com', description: 'メールアドレス' })
      })
    },
    responses: {
      200: {
