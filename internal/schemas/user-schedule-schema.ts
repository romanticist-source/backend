...
export const CreateUserScheduleSchema = z.object({
  userId: z.string().openapi({ example: '123e4567-e89b-12d3-a456-426614174000' }),
  title: z.string().nullable().default(null).openapi({ example: '病院の予約' }),
  description: z.string().nullable().default(null).openapi({ example: '内科の定期検診' }),
  scheduleType: z.string().openapi({ example: 'hospital' }),
  isRepeat: z.boolean().openapi({ example: false }),
  startAt: z.string().openapi({ example: '2024-01-15T10:00:00.000Z' })
...
export const UpdateUserScheduleSchema = z.object({
  ...
  description: z.string().nullable().optional().openapi({ example: '内科の定期検診' }),
  scheduleType: z.string().optional().openapi({ example: 'hospital' }),
  isRepeat: z.boolean().optional().openapi({ example: false }),
  startAt: z.string().optional().openapi({ example: '2024-01-15T10:00:00.000Z' })
...
export const UpdateUserRepeatScheduleSchema = z.object({
  ...
  description: z.string().min(1).optional().openapi({ example: '朝の血圧の薬' }),
