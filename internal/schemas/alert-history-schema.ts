...
export const AlertHistorySchema = z.object({
  ...
  description: z.string().openapi({ example: 'リビングで転倒が検知されました' }),
...
export const CreateAlertHistorySchema = z.object({
  ...
  description: z.string().min(1).openapi({ example: 'リビングで転倒が検知されました' }),
...
export const UpdateAlertHistorySchema = z.object({
  ...
  description: z.string().min(1).optional().openapi({ example: 'リビングで転倒が検知されました' }),
  importance: z.number().int().min(1).max(5).optional().openapi({ example: 3 }),
